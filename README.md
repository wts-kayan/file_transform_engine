...
| `dataquality/DataQualityMapper` | Post-calculation **business** rules on the output (see [Data quality](#data-quality)). |
...
| `job.DataQualityDriver` | Regenerates the **data-quality HTML report** from an already produced CSV (report only). |
...
| `parameters.exclude_ead_ra_rate_ge_1` | drop the full-exposure terms (`EAD_RA_RATE >= 1`) — applied by the data-quality mapper, **after** the rules have seen them |
| `parameters.allow_negative_ead_ra_rate` | let a negative `EAD_RA_RATE` reach the output instead of reporting it as 1 (see [Data quality](#data-quality)) |
...
| `DATA_QUALITY.{enabled,htmlPath,sourcePath,rules.*}` | business data-quality rules + HTML report |
...
`validation/DataControlView`, which checks the **inputs** before the calculation.

| Rule | Checks | Action |
|---|---|---|
| **R01** | lines grouped **by `EAD_MATRIX_ID` and `SCENARIO_ID`** (one group = one curve); flagged when **every** term of the group has `EAD_RA_RATE = 1` — full exposure throughout, so the line carries no information. A single term below 1 keeps the whole group. `EAD_MATRIX_ID` ends in `_Q`/`_Y`, so a matrix's quarterly and yearly curves are two separate groups. | group **removed** from the output and listed in the report |
| **R02** | `EAD_RA_RATE` is strictly negative (an exposure factor must lie in `[0, 1]`) | line **and value kept** as computed, and reported. Optionally masked by a `replaceWith` token for a consumer that cannot take a negative |

The split is deliberate: `dataquality/DataQualityMapper` never writes a row — it evaluates the rules
...
- **`exclude_ead_ra_rate_ge_1` moved.** `PrimaryMapper` now emits *every* computed term; the
  full-exposure exclusion is applied by the data-quality mapper **after** the rules run. Previously it
  deleted those rows inside the mapper, which is precisely what R01 needs to see — an all-ones curve
  had already vanished before any rule could group it. The conf key is unchanged, and so is the
  written output.
- **R02 needs `parameters.allow_negative_ead_ra_rate = true` to ever fire.** By default the run-off
...
- **R02 keeps the value, it does not mask it.** A negative rate can only appear once
...
`DataQualityDriver` re-runs the rules against an already produced CSV and rewrites the report; it
changes nothing. Note it can only report what is still in the file — when the main run already removed
the R01 lines (the default), the report that names them is the one `MainDriver` wrote.

Unit tests: `DataQualityMapperSpec` (rules, tolerance, removal), `DqHtmlViewSpec` (rendering, escaping).
