...
import com.bnp.str.tseadfwd.mapping.{PrimaryMapper, PrimaryView, Term0Row}
...
object Term0Analysis {
  ...
  private def writeCsv(path: String, rows: Seq[Term0Row]): Unit = {
    val f = new File(path)
  ...
  private def writeMarkdown(path: String, rows: Seq[Term0Row]): Unit = {
    val f = new File(path)
    Option(f.getParentFile).foreach(_.mkdirs())
    val pw = new PrintWriter(f, "UTF-8")
  ...
  def renderMarkdown(rows: Seq[Term0Row]): String = {
    val sb = new StringBuilder
    sb.append("# Analysis — Term-0 (period 1) EAD_RA_RATE computation (generated)\n\n")
    sb.append("Auto-generated by the `Term0Analysis` job from the configured inputs — the machine\n")
