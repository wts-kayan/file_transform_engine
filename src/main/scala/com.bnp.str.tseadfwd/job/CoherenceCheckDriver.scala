...
/**
 * Standalone coherence-check REPORT job.
 *
 ...
 * same [[CoherenceCheckMapper]] on the frame it is about to write.
 *
 * Single argument: the path to `application.conf`. Everything else is read from
 * `tseadfwd_app.COHERENCE_CHECK`:
...
object CoherenceCheckDriver {
  ...
  def main(args: Array[String]): Unit = {
    ...
    implicit val spark: SparkSession = SparkSessionManager.fetchSparkSession("tseadfwd-coherence-check")
    ...
    if (!checks.enabled) {
      logger.info("COHERENCE_CHECK.enabled = false -> report skipped.")
      spark.stop()
      return
    }

    logger.info(s"Business coherence check (standalone) reading ${checks.sourcePath}")
    ...
    val report = new CoherenceCheckMapper(checks)
    ...
    println(s"\n>>> Coherence-check report written: ${checks.htmlPath}\n    ${report.summaryLine}")
