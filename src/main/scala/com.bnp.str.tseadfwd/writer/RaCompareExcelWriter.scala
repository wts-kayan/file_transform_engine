...
object RaCompareExcelWriter {
  ...
  private def writeSheet(sheet: XSSFSheet, st: Styles, perimeter: String, fwl: String,
    ...
    writeValueBlock(sheet, st, updated, perimeter, fwl, tables, months, newSeries, st.titleUpdated)
    writeValueBlock(sheet, st, previous, perimeter, fwl, tables, months, oldSeries, st.titlePrevious)
  ...
  private final case class Styles(wb: Workbook) {
    ...
    /** Yellow, as the manual workbook has it — kept for the `Evol` block and the INFO heading. */
    val title: CellStyle = {
    ...
    /**
     * `Updated` blue and `Previous` orange, so the two sides are distinguishable at a glance rather
     * than by reading the word. Light tints, NOT the saturated tab colours: these sit behind black
     * text and have to stay legible. Blue and orange again rather than red and green.
     */
    private def filledTitle(r: Int, g: Int, b: Int): CellStyle = {
      val s = wb.createCellStyle().asInstanceOf[org.apache.poi.xssf.usermodel.XSSFCellStyle]
      s.setFont(bold)
      s.setFillForegroundColor(new XSSFColor(Array(r.toByte, g.toByte, b.toByte), null))
      s.setFillPattern(org.apache.poi.ss.usermodel.FillPatternType.SOLID_FOREGROUND)
      s
    }
    val titleUpdated: CellStyle = filledTitle(0xBD, 0xD7, 0xEE)
    val titlePrevious: CellStyle = filledTitle(0xF8, 0xCB, 0xAD)
    val unit: CellStyle = {
      val s = wb.createCellStyle()
