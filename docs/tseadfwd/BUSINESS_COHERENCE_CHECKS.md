...
```scala
def reportOnly(df, source, runId): CheckReport   // evaluate, touch nothing (rowsOut == rowsIn)
def apply(df, source, runId): CheckOutcome       // evaluate + return the cleaned frame for the writer
```

Order inside `apply`, which matters:

1. CR01 → anti-join the flagged `(EAD_MATRIX_ID, SCENARIO_ID)` keys out (broadcast `left_anti`).
...
3. CR02's marker **last** — it turns the cell into a non-numeric token, so every numeric predicate
...
the output and **CR02 has something to report**.
...
non-numeric cell — including a CR02 `replaceWith` token. It now yields `null`, so the key is reported
